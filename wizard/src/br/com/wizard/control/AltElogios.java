...
@WebServlet("/AltElogios")
public class AltElogios extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		request.getRequestDispatcher("./AltElogios.jsp").forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (ServletFileUpload.isMultipartContent(request)) {
			try {
				...
				FileItem elogiado = (FileItem) itens.get(1);
				FileItem autor = (FileItem) itens.get(2);
				FileItem elogio = (FileItem) itens.get(3);

				Elogios elogios = new Elogios();
				ElogiosDao elogiosDao = new ElogiosDao();

				elogios.setId(Integer.valueOf(id.getString()));
				elogios.setElogiado(elogiado.getString());
				elogios.setAutor(autor.getString());
				elogios.setElogio(elogio.getString());

				String retorno = elogiosDao.alterar(elogios);
				...
				if (retorno.equals("sucesso")) {
					...
					if (!elogios.getMsgErro().equals(null)) {
						request.setAttribute("erro",
								"Não foi possível alterar o Elogio! Erro: " + elogios.getMsgErro());
					}
				}
				RequestDispatcher dis = request.getRequestDispatcher("./AltElogios.jsp?id="+id.getString()+"&elogiado="+elogiado.getString()+"&autor="+autor.getString()+"&elogio="+elogio.getString());
